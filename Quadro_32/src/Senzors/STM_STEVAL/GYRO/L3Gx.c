...
* @retval None.
*/
void L3gxReadRawData(short * pnRawData)
{
  uint8_t buffer[6];
  uint8_t reg;
  ...
  {
	   L3gxByteRead(&buffer[i],L3G_OUT_X_L+i);
  }
 // L3gxBufferRead(buffer,L3G_OUT_X_L, 6);
  ...
  if(!(reg & 0x40))
    {
      for(int i=0; i<3; i++)
        pnRawData[i]=(short)(((uint16_t)buffer[2*i+1] << 8) + buffer[2*i]);
    }
  else
    {
      for(int i=0; i<3; i++)
        pnRawData[i]=((short)((uint16_t)buffer[2*i] << 8) + buffer[2*i+1]);
    }
}
...
* @retval None.
*/
void L3gxReadAngRate(float* pfData,short *offset)
{
  short buffer[3];
  uint8_t reg;
  float fSensitivity;
  ...
  /* offset */
   buffer[0]=(short)(buffer[0]-offset[0]);
   buffer[1]=(short)(buffer[1]-offset[1]);
   buffer[2]=(short)(buffer[2]-offset[2]);
  L3gxByteRead(&reg,L3G_CTRL_REG4);
...
void L3gy_Calibrate(short *offset)
{	
	short GyroXYZ[3];
	#define GAC 5000
...
// 		Suma_z+=Temp_z;
	...
	offset[0]=(short)(Suma_x/GAC);
	offset[1]=(short)(Suma_y/GAC);
	offset[2]=(short)(Suma_z/GAC);
}
