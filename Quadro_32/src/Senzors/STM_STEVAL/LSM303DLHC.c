...
void Lsm303dlhcAccReadOutReg(uint8_t* pcReg)
{
  ...
  {
	  Lsm303dlhcAccI2CBufferRead(&pcReg[i], LSM_A_OUT_X_L_REG_ADDR+i, 1);
  }
...
void Lsm303dlhcAccClickInit(LSMAccClickInit* pxClickInit)
{
  ...
  Lsm303dlhcAccI2CBufferRead( tempReg, LSM_A_CTRL4_REG_ADDR, 5);
 ...
 Lsm303dlhcAccI2CBufferWrite( tempReg, LSM_A_CTRL4_REG_ADDR, 5);
...
//   for (unsigned int i=0;i<6;i++)
//   {
// 	  Lsm303dlhcMagI2CBufferRead(&buffer[i], LSM_M_OUT_X_H_ADDR+1, 1);
//   }
